...
/**
 * 낚시 스팟 마커 그림을 Android drawable / iOS asset catalog 로 복사한다 (Figma 634:1675).
 *
 * 마커는 네이티브(RNCKakaoMapView)에서 Kakao LabelStyle 의 비트맵으로 올라가므로
 * JS 번들의 require() 로는 닿지 않는다. 네이티브 폴더는 prebuild 산출물이라
...
module.exports = function withSpotMarkerDrawable(config) {
  ...
  return withDangerousMod(config, [
    ...
    (dangerousConfig) => {
      ...
      const target = path.join(platformProjectRoot, projectName, 'Images.xcassets', 'spot_marker.imageset');
      fs.mkdirSync(target, { recursive: true });
      fs.copyFileSync(path.join(projectRoot, SOURCE), path.join(target, 'spot_marker.png'));
      fs.writeFileSync(path.join(target, 'Contents.json'), JSON.stringify({
        images: [{ filename: 'spot_marker.png', idiom: 'universal' }],
