...
- [ ] 크기 필수 여부 결정 (서버 `size` 필드 필수 여부 포함)
- [ ] 필수라면 비활성 이유를 화면에 안내, 아니라면 크기 없이 등록 허용

### 10-2. 카메라 권한이 없을 때 안내를 팝업으로

지금은 인증 1단계 미리보기 영역 안에 "카메라 권한이 꺼져 있어요" 텍스트와 링크를 띄운다. 앱 안 팝업으로 바꾼다.
설정 앱으로 보내지 않는 정책(앱 첫 진입 권한 팝업과 같은 방식)을 유지한다.

- `src/app/catch.tsx` — `CaptureStep`의 `placeholder` 영역
- [ ] 권한 없음 안내 팝업 (다시 요청 가능하면 권한 요청, 불가하면 사진 보관함 선택 안내)

### 10-3. 주변 시설 목록 바텀 시트

- `src/features/map/tour-facilities.tsx`
- [ ] 처음 열 때 시트 높이를 낮춘다 (지금은 칩 아래부터 화면 끝까지 `top: 80`으로 덮는다)
- [ ] 시트 뒤에 지도 오버레이(딤)를 깐다
- [ ] 시트가 아래에서 위로 올라오는 애니메이션을 넣는다

### 10-4. 시설 상세도 같은 바텀 시트에서

지금은 목록에서 장소를 누르면 `Modal`로 **다른 바텀 시트가 목록 위에 또 올라온다.**
목록 시트 안에서 상세로 전환해야 한다.

- [ ] 목록 → 상세를 같은 시트 안의 화면 전환으로 변경 (뒤로 가면 목록 복귀)
- [ ] 오버레이는 애니메이션 없이 바로 깔고, 시트만 아래 → 위로 올린다
  (지금 상세 `Modal`은 `animationType="slide"`라 오버레이까지 시트와 함께 올라온다.
  스팟 상세 시트 `spot-detail-sheet.tsx`는 이미 이 방식이다)
