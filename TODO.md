...
### 3-1. SUITE 폰트 — 공통 토큰·네이티브 번들 반영 ✅

- [x] `assets/fonts/`에 SUITE static 6종 추가 (공식 저장소 `sun-typeface/SUITE`, SIL OFL)
...
- [x] `Typography`와 개별 텍스트에 굵기별 SUITE 파일 적용
- [x] #28: `expo-font`로 네이티브 빌드에 6종을 포함하고, Android 별칭 조회를 방해하던 중복 `fontWeight` 제거
- [x] #28: 작은 화면·큰 글씨에서 공통 입력/버튼과 홈 카드가 내용에 맞춰 늘어나도록 수정 (시스템 글자 확대 유지)
- [ ] 실제 Android 기종별 글자 크기 설정과 iOS Dynamic Type 최종 대조 — 검증 범위는 `QA.md` 참고
...
## 10. 인증·주변 시설 UI 후속 이슈 (2026-09-13 실기기 QA)

SM-A165N 개발 빌드에서 제보됐다. #27·#29에서 수정했으며 자동 검사와 기기 검증 범위를 `QA.md`에 구분했다.

### 10-1. 크기를 입력하지 않으면 도감 등록 버튼이 비활성화된다

인증 4단계(634:3140)에서 분류 응답에 크기가 없으면(`sizeCm: null`) 사용자가 크기를 직접 입력하기 전까지
"도감에 등록" 버튼이 눌리지 않는다. 운영 OpenAPI는 일반·수기 등록 모두 `size`를 필수로 선언한다.

- `src/app/catch.tsx` — `canRegister = sizeCm !== null && (!manual || fishName.trim() !== '')`
- `src/features/catch/use-catch-flow.ts` — 등록 요청 전 `sizeCm === null`이면 막는 검증
- [x] 서버 `size` 필수 계약 확인 (`0 < size <= 300`)
- [x] 크기가 없을 때 등록 버튼 위에 크기 입력 안내, 유효한 값을 입력하면 안내 제거

### 10-2. 카메라 권한이 없을 때 안내를 팝업으로

인증 1단계의 권한 안내를 앱 안 팝업으로 제공한다. 닫아도 미리보기에서 다시 열 수 있다.
설정 앱으로 보내지 않는 정책(앱 첫 진입 권한 팝업과 같은 방식)을 유지한다.

- `src/app/catch.tsx` — `CaptureStep`의 `placeholder` 영역
- [x] 권한 없음 안내 팝업 (다시 요청 가능하면 권한 요청, 불가하면 사진 보관함 선택 안내)
- [x] iOS 사진 선택기는 권한 팝업이 닫힌 뒤 한 번만 열기; Android·웹은 사용자 클릭에서 바로 열기
- [x] 작은 화면에서 공통 팝업 내용과 버튼을 스크롤할 수 있도록 높이 제한

### 10-3. 주변 시설 목록 바텀 시트

- `src/features/map/tour-facilities.tsx`
- [x] 처음에는 화면 절반 높이로 열고 펼치기 버튼으로 확장
- [x] 시트 뒤에 지도 오버레이(딤)를 깐다
- [x] 시트만 아래에서 위로 올라오는 애니메이션 (시스템 동작 줄이기 설정 준수)

### 10-4. 시설 상세도 같은 바텀 시트에서

별도 상세 `Modal`을 없애고 목록 시트 안에서 상세로 전환한다. 지도 마커도 같은 시설 상세를 선택한다.

- [x] 목록 → 상세를 같은 시트 안의 화면 전환으로 변경 (뒤로 가면 목록 복귀)
- [x] 오버레이는 애니메이션 없이 바로 깔고, 시트만 아래 → 위로 올린다
